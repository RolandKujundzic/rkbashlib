...
#--
# Print hint (green color message)
#
# @param message
#--
function _hint {
...
function _install_apache2 {
	_apt_update
	_apt_install "apache2 apache2-utils libapache2-mod-php libapache2-mpm-itk"
}
...
if (defined('SETTINGS_DB_NAME') && defined('SETTINGS_DB_PASS')) {
	...
	require(constant('PATH_RKPHPLIB').'db/ADatabase.php');
	$dsn = \rkphplib\db\ADatabase::splitDSN(SETTINGS_DSN);
	$login = $dsn['login'];
	$name = $dsn['name'];
	$pass = $dsn['password'];
}
